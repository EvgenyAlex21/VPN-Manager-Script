...
1.  **Запустите скрипт:**
    ...
    *   Если скрипт не запускается, то скорее всего проблема в правах PowerShell. Откройте              PowerShell от имени администратора и введите следующую команду: `Set-ExecutionPolicy RemoteSigned`. После чего попробуйте запустить скрипт еще раз.
